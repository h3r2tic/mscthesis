...
%TODO: a more fitting picture lolz
\begin{figure}[h!]
  \centering
...
Several post-processing effects were implemented in order to test the pipeline. Figure \ref{fig:postProcOff} shows a test scene. The high-dynamic range output of this rendering is supplied to a \emph{PostProcessor} before gamma-correction. The image is mapped onto a full-screen quadrilateral, which is rendered with shaders generated from a kernel graph. Post-processing effects of multi-scale glare simulation and color banding reduction are applied and yield the image shown in figure \ref{fig:postProcOn}.

\begin{figure}[h!]
  \centering
  \subfigure[Post-processing off]{\label{fig:postProcOff}\includegraphics[width=0.45\linewidth]{./Figures/postProc/off.jpg}}
  \subfigure[Post-processing on]{\label{fig:postProcOn}\includegraphics[width=0.45\linewidth]{./Figures/postProc/glareNoise.jpg}}
...
The most basic post-processing filters are ones which operate uniformly on individual colors. Figure \ref{fig:postProcSimple} demonstrates two such filters.

\subsection{Blur}

As mentioned in section \ref{sec:PostProcessing}, the post-processing systems allows multi-pass rendering. One common application of this feature is computation of separable filters, such as the Gaussian blur, shown in figure \ref{fig:postProcBlur}. The first image shows the result of convolving the image with only a horizontal Gaussian blur. A \emph{Blit} node is inserted after the horizontal blur, followed by vertical blurring. This effectively approximates a complete two-dimensional Gaussian blur, however with an order of magnitude less samples taken from the image.

\begin{figure}[h!]
  \centering
  \subfigure[Horizontal Gaussian blur]{\label{fig:postProcBlurHoriz}\includegraphics[width=0.45\linewidth]{./Figures/postProc/blurMoreHoriz.jpg}}
  \subfigure[Gaussian blur]{\label{fig:postProcBlur}\includegraphics[width=0.45\linewidth]{./Figures/postProc/blurMore.jpg}}
...
\begin{figure}[h!]
  \centering
    ...
    \caption[Glare]{Multi-scale glare applied to the test scene}
...
\begin{figure}[h!]
  \centering
  \subfigure[Banding artifact]{\label{fig:postProcBanding}\includegraphics[width=0.45\linewidth]{./Figures/postProc/banding.png}}
